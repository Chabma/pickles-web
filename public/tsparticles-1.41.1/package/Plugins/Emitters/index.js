...
class EmittersPlugin {
  ...
  getPlugin(container) {
    return new Emitters_1.Emitters(
      __classPrivateFieldGet(this, _EmittersPlugin_engine, "f"),
      container
    );
  }
  needsPlugin(options) {
  ...
  loadOptions(options, source) {
    ...
    if (interactivityEmitters) {
      if (interactivityEmitters instanceof Array) {
        optionsCast.interactivity.modes.emitters = interactivityEmitters.map(
          (s) => {
            const tmp = new Emitter_1.Emitter();
            tmp.load(s);
            return tmp;
