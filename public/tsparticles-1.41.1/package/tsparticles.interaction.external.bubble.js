  ...
  return (function () {
    ...
    class Bubbler extends ExternalInteractorBase {
      ...
      isEnabled() {
        ...
        return (
          Utils_isInArray("bubble", hoverMode) ||
          Utils_isInArray("bubble", clickMode) ||
          divBubble
        );
      }
      ...
      singleSelectorHover(selector, div) {
        ...
        selectors.forEach((item) => {
          const elem = item,
            ...
            area =
              div.type === "circle"
                ? new Circle_Circle(pos.x, pos.y, repulseRadius)
      ...
      clickBubble() {
        ...
        for (const particle of query) {
          ...
          const pos = particle.getPosition(),
            distMouse = NumberUtils_getDistance(pos, mouseClickPos),
            timeSpent =
              (new Date().getTime() -
          ...
          const sizeData = {
            ...
            particlesObj: {
              optValue:
                NumberUtils_getRangeMax(particle.options.size.value) *
                container.retina.pixelRatio,
      ...
      hoverBubble() {
        ...
        for (const particle of query) {
          particle.bubble.inRange = true;
          const pos = particle.getPosition(),
            pointDistance = NumberUtils_getDistance(pos, mousePos),
            ratio = 1 - pointDistance / distance;
          if (pointDistance <= distance) {
            if (
              ratio >= 0 &&
              container.interactivity.status ===
                Constants_Constants.mouseMoveEvent
            ) {
          ...
          if (
            container.interactivity.status ===
            Constants_Constants.mouseLeaveEvent
          ) {
            this.reset(particle);
          }
        }
      ...
      hoverBubbleOpacity(particle, ratio, divBubble) {
        ...
        const opacity = calculateBubbleValue(
          pOpacity,
          modeOpacity,
          NumberUtils_getRangeMax(optOpacity),
          ratio
        );
      ...
      hoverBubbleColor(particle, ratio, divBubble) {
        ...
        if (!particle.bubble.finalColor) {
          ...
          const bubbleColor =
            modeColor instanceof Array
              ? Utils_itemFromArray(modeColor)
              : modeColor;
