(function (factory) {
  ...
  class Bubbler extends Core_1.ExternalInteractorBase {
    ...
    isEnabled() {
      ...
      return (
        (0, Utils_1.isInArray)("bubble", hoverMode) ||
        (0, Utils_1.isInArray)("bubble", clickMode) ||
        divBubble
      );
    }
    ...
    async interact() {
      ...
      if (hoverEnabled && (0, Utils_1.isInArray)("bubble", hoverMode)) {
        ...
        (0, Utils_1.divModeExecute)("bubble", divs, (selector, div) =>
    ...
    clickBubble() {
      ...
      for (const particle of query) {
        ...
        const sizeData = {
          ...
          particlesObj: {
            optValue:
              (0, Utils_1.getRangeMax)(particle.options.size.value) *
    ...
    hoverBubble() {
      ...
      for (const particle of query) {
        ...
        if (pointDistance <= distance) {
          if (
            ratio >= 0 &&
            container.interactivity.status === Core_1.Constants.mouseMoveEvent
          ) {
        ...
        if (
          container.interactivity.status === Core_1.Constants.mouseLeaveEvent
        ) {
          this.reset(particle);
        }
      }
    ...
    hoverBubbleOpacity(particle, ratio, divBubble) {
      ...
      const opacity = calculateBubbleValue(
        pOpacity,
        modeOpacity,
        (0, Utils_1.getRangeMax)(optOpacity),
        ratio
      );
      ...
      if (!particle.bubble.finalColor) {
        ...
        const bubbleColor =
          modeColor instanceof Array
            ? (0, Utils_1.itemFromArray)(modeColor)
            : modeColor;
