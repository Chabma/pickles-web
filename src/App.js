...
class App extends Component {
  ...
  */
    ...
    this.setState({total_queue: tempQueue},
        () =>{
            if(playBoolean){this.play(position)}
            this.getRecs();
        }
    );   
      ...
      success: (data) => {
        if(data){ 
            for(var i = 0; i < data.audio_features.length; i++){
                ...
                additionalFeatureValues[additionalFeatures[j]] = additionalFeatureValues[additionalFeatures[j]] ?? 0;
                additionalFeatureValues[additionalFeatures[j]] += data.audio_features[i][additionalFeatures[j]];
                }
            }
            for(j = 0; j < additionalFeatures.length; j++){
                let divisor = Math.min( data.audio_features.length, 25);
                additionalFeatureValues[additionalFeatures[j]] = additionalFeatureValues[additionalFeatures[j]] / divisor;
            }
        }
        ...
        for(j = 0; j < additionalFeatures.length; j++){
            additionalFeatureString += "&target_" +additionalFeatures[j]+"="+additionalFeatureValues[additionalFeatures[j]];        
            stateAdditionalFeatureString += "  "+additionalFeatures[j]+" : "+additionalFeatureValues[additionalFeatures[j]];
        }

        if(primaryFeature === "song"){
            artists_string = "seed_artists="+all_artists[all_artists.length - 1];
        }
        else{
        ...
        $.ajax({
          ...
          success: (data) => {
            if(data){ 
                ...
                let recSongs = [];
                let h = 0;
                let shuffled = data.tracks.map(value => ({ value, sort: Math.random() })).sort((a, b) => a.sort - b.sort).map(({ value }) => value);
                  ...
                  success: (data) => {
                    if(data){ 
                        ...
                        for(var i = 0; i< data.audio_features.length; i++){
                            next_features[i] = ""
                            for(var j = 0; j < additionalFeatures.length; j++){
                                next_features[i] += " " + additionalFeatures[j] +" : "+ data.audio_features[i][additionalFeatures[j]];
                            }
                        }
    ...
    return ( 
      <div className="App">
        ...
        <div className="App-header">
          ...
          {!code.code && (
          <>
          <meta http-equiv="Refresh" content={`0; url='${authEndpoint}?client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scopes.join("%20")}&response_type=code&show_dialog=false'`} />
          ...
          {code.code && (
              <div style={{zIndex: 10, width: "100%", height: "100%"}}>
                  ...
                  <div className="Search">
                      <input
                          id="searchInput" 
                          style={{color: 'black',fontFamily: "Roboto",width: "100%"}}
...
> Save to Spotify:</h4>
                     <Search
                        ...
                        onSearch={() => {
                            this.add_playlist_btn_func(document.getElementById("playlist_name").value, this.state.userID );
                        }}
                      />
                  </div>
                  <br />
